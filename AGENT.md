...
| `eslint` | Bash | allow `[npx] eslint <paths/flags>` — read-only lint, no required subcommand (bare `eslint <paths>` lints and exits); reject the write flags `--fix`/`--fix-dry-run` (rewrite source) and `--init` (scaffolds config); (+ cd && variant, pipe to consumers) | — |
