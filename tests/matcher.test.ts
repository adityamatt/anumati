...
const ALLOW_API_WITH_PYTHON: Rule = {
  ...
  desc: "example API with HTML parsing",
};
...
describe("evaluate — curl piped to python3", () => {
  ...
  it("returns null for curl | python3 when rule has no allowed_imports", () => {
    const rule: Rule = { tool: "Bash", matcher: "curl", allowed_domains: ["api.example.com"] };
    const cmd = `curl -s "https://api.example.com/p/" | python3 -c "import sys; print(sys.stdin.read())"`;
...
const ALLOW_GH: Rule = {
  ...
  allowed_repos: ["octocat/example-repo"],
  desc: "example-repo API reads",
};
...
describe("evaluate — gh matcher", () => {
  it("allows gh api for allowed repo", () => {
    const cmd = `gh api "repos/octocat/example-repo/contents/README.md" --jq '.content' | base64 -d | grep -A2 "liked" | head -80`;
  ...
  it("returns null for write method", () => {
    const cmd = `gh api "repos/octocat/example-repo/issues" --method POST --field title=x`;
    expect(evaluate(bash(cmd), [ALLOW_GH]).decision).toBeNull();
  });

  it("returns null for -X POST", () => {
    const cmd = `gh api "repos/octocat/example-repo/issues" -X POST`;
    expect(evaluate(bash(cmd), [ALLOW_GH]).decision).toBeNull();
  });

  it("allows --method GET explicitly", () => {
    const cmd = `gh api "repos/octocat/example-repo/contents/README.md" --method GET`;
    expect(evaluate(bash(cmd), [ALLOW_GH]).decision).toBe("allow");
  });
  ...
  it("returns null for && chained commands", () => {
    expect(evaluate(bash(`gh api "repos/octocat/example-repo/contents/README.md" && rm -rf /`), [ALLOW_GH]).decision).toBeNull();
  });
  ...
  it("allows gh api piped to python3-c with allowed imports", () => {
    ...
    const cmd = `gh api "repos/octocat/example-repo/contents/README.md" --jq '.content' | python3 -c "import sys, json; print(sys.stdin.read())"`;
    expect(evaluate(bash(cmd), [rule]).decision).toBe("allow");
  });

  it("returns null for gh api piped to python3-c with unlisted import", () => {
    const cmd = `gh api "repos/octocat/example-repo/contents/README.md" | python3 -c "import os; os.system('id')"`;
    expect(evaluate(bash(cmd), [ALLOW_GH]).decision).toBeNull();
  });
});
...
describe("evaluate — npx vite build / tsc via build-tool + npx-tsc", () => {
  ...
  const CD: Rule = { tool: "Bash", matcher: "cd", open: { allowed_paths: ["/Users/you"] } };
  const rules = [BUILD, TSC, CD];

  function bashCwd(command: string): HookInput {
    return { session_id: "t", tool_name: "Bash", tool_input: { command }, cwd: "/Users/you" };
  }
