...
describe("matchAws — s3 / s3api (allow pure reads)", () => {
  ...
  it("s3api get-bucket-encryption", () =>
    expect(
      matchAws('aws s3api get-bucket-encryption --profile myapp-prod --region us-east-1 --bucket myapp-data-prod-111122223333-us-east-1'),
    ).toBe(true));
...
describe("matchAws — sts (allow read-only)", () => {
  it("get-caller-identity", () =>
    expect(matchAws("aws sts get-caller-identity --profile myapp-prod")).toBe(true));
});
...
describe("matchAws — iam (allow read-only)", () => {
  it("list-role-policies", () =>
    expect(matchAws('aws iam list-role-policies --profile myapp-prod --role-name "MyAppStack-query-worker-role" --output text')).toBe(true));
  ...
  it("get-role-policy", () =>
    expect(matchAws('aws iam get-role-policy --role-name "MyAppStack-query-worker-role" --policy-name P --output json')).toBe(true));
...
describe("matchAws — dynamodb (allow read-only)", () => {
  ...
  it("scan with filter/projection flags", () => {
    expect(
      matchAws(
        `aws dynamodb scan --profile myapp-prod --region eu-central-1 --table-name "MyAppStack-config" --filter-expression "begins_with(sk, :f)" --expression-attribute-values '{":f":{"S":"file#"}}' --output json`,
      ),
    ).toBe(true);
  });
...
describe("matchAws — lambda (allow read-only)", () => {
  it("get-function-configuration with query flags", () => {
    expect(
      matchAws(
        `aws lambda get-function-configuration --profile myapp-prod --region eu-central-1 --function-name "MyAppStack-QueryRunnerRunFileHandler60586-THdlLFGiJ3R3" --query "{VpcConfig:VpcConfig,Timeout:Timeout}" --output json`,
      ),
    ).toBe(true);
  });
...
describe("matchAws — logs start-query (allow read-only query dispatch)", () => {
  it("start-query with --output text", () => {
    expect(
      matchAws(
        `aws logs start-query --profile myapp-prod --region eu-central-1 --log-group-name "/aws/lambda/MyAppStack-TieredProcessorSmallHandlerA33-uVw6AyovyDmX" --start-time 1784181612 --end-time 1784192412 --query-string 'fields @timestamp, @message | filter @message like /out of memory/ | sort @timestamp desc | limit 50' --output text`,
      ),
    ).toBe(true);
  });
...
describe("matchAws — cloudwatch (allow read-only)", () => {
  it("list-metrics", () => {
    expect(
      matchAws(
        `aws cloudwatch list-metrics --profile myapp-prod --region eu-central-1 --namespace MyMetrics --dimensions Name=File,Value=path_actual_cpt --output json`,
      ),
    ).toBe(true);
  });
  it("get-dashboard with --output json", () => {
    expect(
      matchAws(
        `aws cloudwatch get-dashboard --profile myapp-prod --region eu-central-1 --dashboard-name "MyMetrics--prod--path_actual_cpt" --output json`,
      ),
    ).toBe(true);
  });
  it("get-dashboard | head", () => {
    expect(
      matchAws(
        `aws cloudwatch get-dashboard --dashboard-name "MyMetrics--prod--path_actual_cpt" --output json | head`,
      ),
    ).toBe(true);
  });
...
describe("matchAws — cloudwatch (block writes / alarm mutations)", () => {
  ...
  it("get-dashboard | python3 (unsafe pipe consumer) stays rejected", () => {
    expect(
      matchAws(
        `aws cloudwatch get-dashboard --profile myapp-prod --region eu-central-1 --dashboard-name "MyMetrics--prod--path_actual_cpt" --output json 2>&1 | python3 -c "import sys,json; d=json.load(sys.stdin); print(d)"`,
      ),
    ).toBe(false);
  });
});
...
describe("matchAws — account (allow region/contact reads)", () => {
  it("list-regions with query/output flags", () => {
    expect(
      matchAws(
        `aws account list-regions --profile myapp-prod --region-opt-status-contains ENABLED ENABLED_BY_DEFAULT --query 'Regions[].RegionName' --output text`,
      ),
    ).toBe(true);
  });
  it("list-regions | head", () => {
    expect(
      matchAws(
        `aws account list-regions --profile myapp-prod --region-opt-status-contains ENABLED ENABLED_BY_DEFAULT --query 'Regions[].RegionName' --output text 2>&1 | head`,
      ),
    ).toBe(true);
  });
...
describe("matchAws — configure (allow local config reads)", () => {
  ...
  it("get region --profile", () => expect(matchAws("aws configure get region --profile myapp-prod")).toBe(true));
