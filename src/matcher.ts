...
/**
 * Split a command into sub-commands at top-level `&&` and `;` only — the purely
 * *sequential* operators, where no data flows between the pieces. Pipes are kept
 * INSIDE each sub-command (rejoined with ` | `), so a piped chain is always
 * handed to a single matcher as one unit and never composed across rules — the
 * pipe is a data-flow channel whose safety only the receiving matcher can judge.
 *
...
export function decomposeSequential(command: string): string[] | null {
  ...
  for (const s of segments) {
    if (s.operator === "||" || s.operator === "&") return null;
  }
  ...
  for (const s of segments) {
    group.push(s.raw);
    // `&&` / `;` end the current sub-command; `|` (or null) keep accumulating.
