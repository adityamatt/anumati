...
| `allowed_remotes`    | `git-push`                    | Remotes `git push [-u] <remote> <branch>` may target (empty = disabled); force/delete/bulk pushes and protected-branch targets always blocked              |
| `protected_branches` | `git-push`                    | Branch names a push must never target — ADDITIVE with the built-in `main`/`master`/`release`/`production`/`prod` (can extend, never shrink)                 |
...
| `open.allowed_paths` | `python3-pipe`, `nodejs-pipe`, `cd`, `node-script` | Path prefixes: for `python3-pipe`/`nodejs-pipe`, files a script may read (`open()` / file-path `require()`); for `cd`, extra directory roots a `cd` may target (beyond the cwd); for `node-script`, the code roots a runnable script must live under (empty = disabled) |
...
| `python3-pipe` | Bash | allow `python3 -c` or `python3 script.py [args…]` (file contents validated like inline code) with allowlisted imports, no dangerous builtins                                                                                                       | `allowed_imports`, `open.allowed_paths` |
| `nodejs-pipe`  | Bash | allow `node -e`/`-p` or `node script.js [args…]` (file contents validated like inline code) with allowlisted built-in modules, no `fs`/network/`child_process`/`eval`; a file-path `require()` is allowed only under `open.allowed_paths`          | `allowed_modules`, `open.allowed_paths` |
...
| `aws`          | Bash | allow read-only AWS CLI for supported services (`logs`, `stepfunctions`, `s3`/`s3api`, `dynamodb`, `lambda`, `sts`, `iam`) — list/describe/get/query/scan/filter only; writes and local-write commands (`s3 cp`/`sync`/`rm`, `s3api get-object`, `dynamodb put`/`update`/`delete`/`execute-statement`, `lambda invoke`/`update`/`delete`, `sts assume-role`/`get-session-token`, all `iam` create/update/delete/attach/put) blocked (+ `cd … &&` variant, pipe to builtins) | —                                       |
...
| `git-push`     | Bash | allow only `git push [-u] <remote> <branch>` to an allowlisted remote and non-protected branch; force (`--force`/`-f`/`+refspec`), delete, bulk (`--all`/`--mirror`/`--tags`), `--no-verify`, and protected-branch targets always blocked           | `allowed_remotes`, `protected_branches` |
| `gh-pr`        | Bash | allow non-destructive `gh pr` subcommands (create/edit/comment/view/list/…); `merge`/`close`/`reopen`/`review` blocked                                                                                                                             | —                                       |
| `node-script`  | Bash | allow running a TRUSTED local script by location — `node <script> [args…]` whose resolved path sits inside `open.allowed_paths` (cwd counts as a bonus root once a path is set); runtime flags (`-r`/`--import`/…) and file redirection blocked      | `open.allowed_paths`                    |
