...
| `aws`          | Bash | allow read-only AWS CLI for supported services (`logs`, `stepfunctions`, `s3`/`s3api`, `dynamodb`, `lambda`) — list/describe/get/query/scan/filter only; writes and local-write commands (`s3 cp`/`sync`/`rm`, `s3api get-object`, `dynamodb put`/`update`/`delete`/`execute-statement`, `lambda invoke`/`update`/`delete`) blocked (+ `cd … &&` variant, pipe to builtins) | —                                       |
